    ...
    protected function processStatus()
    {
        ...
        try {
            ...
            switch ($status) {
                ...
                case "partial":
                case "authorizing":
                    Tools::redirect('index.php?controller=order-confirmation&id_cart='.(int)($id).'&id_module='.(int)($this->module->id).'&id_order='.$order->id.'&key='.$customer->secure_key);
                    break;
                case "failed":
                    Tools::redirect('index.php?controller=order-confirmation&id_cart='.(int)($id).'&id_module='.(int)($this->module->id).'&id_order='.$order->id.'&key='.$customer->secure_key);
					...
					if (!$order->id) {
                        ...
                        if ($cart->getOrderTotal() == $body["base_price"]) {
                            if ($order_currency->iso_code == $body["base_ccy"]) {
                                ...
                                YellowBitcoinDbLog::log($id, "Currency of order {$order->id} is wrong. Order currency: " . $order_currency->iso_code . "; paid: " . $body["base_ccy"]);
                            }
                        } else {
                            YellowBitcoinDbLog::log($id, "Paid amount of order {$order->id} is wrong. Order amount: " . $cart->getOrderTotal() . "; paid: " . $body["base_price"]);
                        }
                    }
                    break;
                case 'reissue':
					YellowBitcoinDbLog::log($id, "Client re-issued the invoice. Invoice Id: " . $body['id']); //this must be changed when we had reissue / renew payment ready
                    break;
                case 'partial':
					YellowBitcoinDbLog::log($id, "Client made a partial payment. Invoice Id: " . $body['id']); //this must be changed when we had reissue / renew payment ready
                    break;
                case 'failed':
                case 'invalid':
