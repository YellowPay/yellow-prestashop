...
class YellowBitcoinIpn extends ObjectModel
{
    ...
    public static function checkInvoiceStatus($id)
    {
        ...
        if (!is_array($data)) {
            throw new Exception('We\'re sorry, an error has occurred while completing your request. Please refresh the page to try again. If the error persists, please send us an email at support@yellowpay.co\n line');
        }
        ...
        switch ($data['status']) {
            ...
            case $data["status"] == "authorizing":
                ...
                $module->validateOrder((int)$id_cart, Configuration::get('YP_OS_WAITING'), $total, $module->displayName, NULL, array('transaction_id' => $data["id"] . ' (' . $data["invoice_price"] . ' BTC)'), (int)$cart->id_currency, false, $customer->secure_key);
                break;
