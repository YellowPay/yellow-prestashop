...
class Yellow_Bitcoin extends PaymentModule
{
	...
	public function __construct()
	{
		...
		$this->description = $this->l('Accept payments in Bitcoin from anywhere in the world; and receive settlements in your local currency to your bank account');
	...
	public function hookHeader()
	{
		...
		if (!in_array($this->context->controller->php_self, array('order-opc', 'order')) || (isset($this->context->controller->page_name) && $this->context->controller->page_name != 'module-yellow_bitcoin-payment'))
			return;
    ...
    public function createYellowInvoice($quote)
    {
		...
		$ipnUrl = $this->context->link->getModuleLink('yellow_bitcoin', 'default', array('action' => 'ipn', 'id' => base64_encode($quote->id)), true);
        ...
        try {
            ...
            if ($http_status == "200") {
                ...
                YellowBitcoinDbLog::log($quote->id, 'Error code response received: ' . $http_status . '; Response body: ' . Tools::jsonEncode($response) . "; id cart:" . pSQL($quote->id));

                return false;
            }
        } catch (Exception $exc) {
            YellowBitcoinDbLog::log($quote->id, 'EXCEPTION: ' . $exc->getMessage() . '; Response body: ' . Tools::jsonEncode($response) . "; id cart:" . pSQL($quote->id));

            return $this->l('We\'re sorry, an error has occurred while completing your request. Please refresh the page to try again. If the error persists, please send us an email at support@yellowpay.co');
        }
    }
